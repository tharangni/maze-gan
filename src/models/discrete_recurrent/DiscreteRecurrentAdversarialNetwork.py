...
class DiscreteRecurrentAdversarialNetwork:
    ...
    def __init__(self, opts):
        self.name = 'RecurrentHeavisideModel'
        now = datetime.today().strftime('%Y-%m-%d/%H-%M-%S')
        self.writer = SummaryWriter(log_dir=os.path.join('models', 'discrete_recurrent', 'runs', now))
        ...
        self.generator_opts = Bunch(
            ...
            resume=opts.resume, temp=opts.temp, name=self.name, writer=self.writer
        )
        self.discriminator_opts = Bunch(
            ...
            resume=opts.resume, name=self.name, writer=self.writer
        )
        ...
        # self.writer.add_graph(self.G, (torch.rand(self.batch_size, self.mx, self.my)))

    def train(self):
        # --- Maze Data --- #
        print('Generating {} {}x{} maze examples.'.format(self.N, self.mx, self.my))
        maze_data = gen_maze_data(self.N, self.mx, self.my)
        torch.save(maze_data, os.path.join(self.results_path, 'real_mazes.pickle'))

        data_loader = maze_data \
            .reshape(-1, self.batch_size, self.mx, self.my) \
            .to(device=self.device, dtype=torch.float32)
        # -- Number of batches -- #
        num_batches = len(data_loader)

        # -- Generate labels -- #
        real = torch.ones([self.batch_size, 1], dtype=torch.float).to(device=self.device, dtype=torch.float32)
        fake = torch.zeros([self.batch_size, 1], dtype=torch.float).to(device=self.device, dtype=torch.float32)

        # --- Start training --- #
        for epoch in range(self.num_epochs):
            all_fake_mazes = None
            for batch_idx, real_mazes in enumerate(data_loader):
                real_mazes = real_mazes.reshape(self.batch_size, -1)
                ...
                # -- Train Discriminator -- #
                z = torch.rand(self.batch_size, self.latent_size).to(device=self.device, dtype=torch.float32)
                ...
                # -- Save run results -- #
                if (epoch + 1) % 1 == 0:
                    current_iter = num_batches * epoch + batch_idx + 1
                    self.writer.add_scalar('Generator/loss', g_loss.item(), current_iter)
                    self.writer.add_scalar('Discriminator/loss', d_loss.item(), current_iter)
                    self.writer.add_scalar('D(x)', d_real_scores.mean().item(), current_iter)
                    self.writer.add_scalar('D(G(z))', g_fake_scores.mean().item(), current_iter)

                    for name, param in self.G.named_parameters():
                        self.writer.add_histogram("Generator/" + name, param.clone().cpu().data.numpy(), current_iter)
                    for name, param in self.D.named_parameters():
                        self.writer.add_histogram("Discriminator/" + name, param.clone().cpu().data.numpy(),
                                                  current_iter)

            if (epoch + 1) % 1 == 0:
                torch.save(all_fake_mazes, os.path.join(self.results_path, 'fake_mazes_{}.pickle'.format(epoch + 1)))
