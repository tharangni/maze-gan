...
from models.continuous_vanilla.Generator import Generator
from models.continuous_vanilla.Discriminator import Discriminator
...
class GeneralAdversarialNetwork:
    ...
    def __init__(self, opts):
        ...
        self.name = 'ContinuousVanillaMNIST'

        now = datetime.today().strftime('%Y-%m-%d/%H-%M-%S')
        self.writer = SummaryWriter(log_dir=os.path.join('models', 'continuous_vanilla', 'runs', now))
        ...
        self.generator_opts = Bunch(
            ...
            name=self.name, writer=self.writer
        )
        self.discriminator_opts = Bunch(
            ...
            batch_size=opts.batch_size, learning_rate=opts.d_lr, resume=opts.resume, name=self.name, writer=self.writer
        )
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_idx, real_images in enumerate(data_loader):
                ...
                if (epoch + 1) % 1 == 0:
                    ...
                    self.writer.add_scalar('D(x)', real_scores.mean().item(), current_iter)
                    self.writer.add_scalar('D(G(z))', fake_scores.mean().item(), current_iter)

                    for name, param in self.G.named_parameters():
                        self.writer.add_histogram("Generator/" + name, param.clone().cpu().data.numpy(),
                                                  current_iter, bins='auto')
            ...
            save_image(denorm(fake_images), os.path.join(self.results_path, 'fake_images-{}.png'.format(epoch + 1)))
