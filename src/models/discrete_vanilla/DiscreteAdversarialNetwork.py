...
from models.discrete_vanilla.Generator import Generator
from models.discrete_vanilla.Discriminator import Discriminator
from helpers.Bunch import Bunch


class DiscreteAdversarialNetwork:
    ...
    def __init__(self, opts):
        ...
        self.my = opts.my
        self.N = opts.N
        self.results_path = os.path.join('results', 'VanillaMazes')
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            Checkpoint(generator_checkpoint, 'VanillaMazes', 'generator').save()
            discriminator_checkpoint = Bunch(
                epoch=epoch + 1, state_dict=self.D.model.state_dict(),
                optimizer=self.D.optimizer.state_dict()
            )
            Checkpoint(discriminator_checkpoint, 'VanillaMazes', 'discriminator').save()
