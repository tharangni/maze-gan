...
def gen_maze_data(N, mx, my, save_to_file=False, dir="NA", randomise=False):
    data = []
    for i in range(N):
        if randomise:
            mx = random.randint(2, 51)
            my = mx
        maze = generate_maze(mx, my)
        if save_to_file: draw_maze(maze, save_to_file=save_to_file, num=i, dir=dir)
        if i % 100 == 0:
