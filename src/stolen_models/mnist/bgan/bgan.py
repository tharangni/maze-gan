...
mnist_loader = torch.zeros_like(mnist.train_data).type(Tensor)
for idx in range(len(mnist)):
    mnist_loader[idx], _ = mnist[idx]
mnist_loader = mnist_loader.reshape(-1, opt.batch_size, 1, 28, 28)
    ...
    for i, imgs in enumerate(mnist_loader):
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            for name, param in generator.named_parameters():
                writer.add_histogram("Generator/" + name, param.clone().cpu().data.numpy(),
                                     batches_done, bins='auto')
            for name, param in discriminator.named_parameters():
                writer.add_histogram("Discriminator/" + name, param.clone().cpu().data.numpy(),
                                     batches_done, bins='auto')
