...
class GAN:
    ...
    def __init__(self,
        ...
        self.D = Discriminator(self.device, hidden_size, mx*my, mx, num_epochs, batch_size)
        if device == 'cuda':
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for local_batch , maze_set in enumerate(data_loader):
                ...
                #maze_set = maze_set.to(self.device).float()
                ...
                fake_labels = 0 + torch.rand([self.batch_size, 1], dtype=torch.float).to(self.device) * (0.1 - 0.0)
                real_labels = 0.9 + torch.rand([self.batch_size, 1], dtype=torch.float).to(self.device) * (1.0 - 0.9)
#
                noise_samples = 5

                if (epoch % noise_samples == 0):
                    fake_labels = 0.9 + torch.randn([self.batch_size, 1], dtype=torch.float).to(self.device) * (
                            1.0 - 0.9)
                ...
                #Train Discrimator
                d_loss, fake_score, real_score, fake_mazes = self.D.forward(self.G,
                                                                            self.G.input_size,
                                                                            maze_set,
                                                                            loss_criterion,
                                                                            real_labels,
                ...
                if (epoch % noise_samples == 0):
                    real_labels = 0.9 + torch.rand([self.batch_size, 1], dtype=torch.float).to(self.device) * (1.0 - 0.9)
                g_loss = self.G.forward(self.D, loss_criterion, real_labels, self.reset_grad)
