...
def start():
    ...
    parser.add_argument('--mx', help='No. columns in maze', type=int, default=16)
    parser.add_argument('--my', help='No. rows in maze', type=int, default=16)
    ...
    if args.v:
        ...
        # create folder with mazes
        data = gen_maze_data(5000, 16, 16)
        folder = 'realmazes'
        if not os.path.exists(folder):
            os.makedirs(folder)
        i = 0
        for x in data:
            filename = os.path.join('realmazes/maze-{}'.format(i))
            np.save(filename, x)
            i += 1
